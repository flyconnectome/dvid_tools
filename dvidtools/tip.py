...
def _branch_length(x, g):
    """ Returns length of path from node x to the first branch point. """
    return nx.shortest_path_length(g, x, _first_branch_point(x, g), weight='weight')


def _tortuosity(x, g):
    """ Returns tortuosity for path from node x to the first branch point."""
...
def _dist_to_hk(x):
    """ Returns distance to closest hot knife section. """
...
def _dot_product(x, g, normal=True):
    """ Returns dot product of (normalised) vector of x and the first downstream branch point. """
    if g.out_degree(x) == 0:
        return None
...
def _surround_radius(x, g, n=5):
    """ Returns mean radius in the surrounding of a node. """
    r = []
...
def _dist_to_psd(swc, syn):
    """ Returns distance to next PSD for all rows in a SWC table."""
    post = syn[syn.Kind=='PostSyn']
...
def _dist_to_tbar(swc, syn):
    """ Returns distance to next presynapse for all rows in a SWC table."""
...
def _dist_to_tip(tips):
    """ Returns distance to next closest tip for all rows in tips table."""
...
def _in_out_ratio(swc, syn, r=1000):
    """ Returns ratio of PSDs vs presynapses within given radius. """
    post = syn[syn.Kind=='PostSyn']
